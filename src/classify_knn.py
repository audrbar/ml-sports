...
from utils import adjust_pandas_display, split_data_to_two, DATA_DIR, MODELS_DIR
...
KNN_MODEL = os.path.join(MODELS_DIR, "knn_trained_model.pkl")
KNN_ENCODER = os.path.join(MODELS_DIR, "knn_label_encoder.pkl")
...
def load_data(_file_path):
    ...
    try:
        ...
        if df.isna().sum().sum() > 0:
            print(f"Warning: Missing values found in {_file_path}. Dropping them.")
            df = df.dropna()
        ...
        _y = _label_encoder.fit_transform(df["category"])
        return _X, _y, _label_encoder  # Return encoder too
...
def save_model_and_encoder(model, label_encoder_, model_path, encoder_path):
    """Saves the trained KNeighborsClassifier model and label encoder."""
    # Save model using joblib
    joblib.dump(model, model_path)
    # Save label encoder using pickle
    with open(encoder_path, "wb") as f:
        pickle.dump(label_encoder_, f)
...
if __name__ == "__main__":
    ...
    for dataset_key, file_path in DATA_FILES.items():
        ...
        X, y, label_encoder = load_data(file_path)  # Also retrieve label_encoder
        ...
        save_model_and_encoder(grid_search.best_estimator_, label_encoder, KNN_MODEL, KNN_ENCODER)
...
#     "FastText": os.path.join(DATA_DIR, "articles_vectorized_fasttext.pkl"),
# }
# KNN_MODEL = os.path.join(MODELS_DIR, "knn_trained_model.keras")
# KNN_ENCODER = os.path.join(MODELS_DIR, "knn_label_encoder.pkl")
#
#
# def load_data(_file_path):
...
#         results[dataset_key] = test_scores
#
#     save_model_and_encoder(model, label_encoder, KNN_MODEL, KNN_ENCODER)
#     print("\nFinal KNeighbors Classifier Performance Comparison Across Different Vectorization Methods:")
#     print(pd.DataFrame(results, index=["Accuracy", "Precision", "Recall", "F1 Score"]))
#     plot_metrics_comparison(results)
